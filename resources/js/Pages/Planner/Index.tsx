...
export default function PlannerIndex({ schools, masterMenus, dailyMenus }: PlannerProps) {
    ...
    const handleBulkDelete = () => {
        if (confirm(`Hapus ${selectedIds.length} rencana menu terpilih?`)) {
            router.post(route('planner.bulkDestroy'), { ids: selectedIds }, {
                    ...
                    <div className="lg:col-span-2">
                        <section className="bg-slate-900 rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden min-h-[500px]">
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 relative z-10">
                                <h3 className="text-xl font-black font-headline">Agenda Distribusi</h3>
                                ...
                                {dailyMenus.length > 0 && (
                                    <div className="flex items-center gap-3">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                if (selectedIds.length === dailyMenus.length) {
                                                    setSelectedIds([]);
                                                } else {
                                                    setSelectedIds(dailyMenus.map(dm => dm.id));
                                                }
                                            }}
                                            className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs font-bold transition-all"
                                        >
                                            {selectedIds.length === dailyMenus.length ? 'Batal Pilih Semua' : 'Pilih Semua'}
                                        </button>
                                        ...
                                        {selectedIds.length > 0 && (
                                            <button
                                                ...
                                                className="px-4 py-2 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-xs font-black uppercase tracking-wider transition-all flex items-center gap-2 shadow-lg shadow-rose-500/20"
                                            >
                                                <span className="material-symbols-outlined text-xs">delete_sweep</span>
                                                Hapus {selectedIds.length} Rencana
                                            </button>
                                        )}
                                    </div>
                                )}
                                        ...
                                        <div key={dm.id} className="bg-white/5 border border-white/5 p-6 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 group hover:bg-white/10 transition-all">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div 
                                                    onClick={() => {
                                                        setSelectedIds(prev => 
                                                            prev.includes(dm.id) ? prev.filter(id => id !== dm.id) : [...prev, dm.id]
                                                        );
                                                    }}
                                                    className={cn(
                                                        "w-6 h-6 rounded-xl border flex items-center justify-center cursor-pointer transition-all shrink-0",
                                                        selectedIds.includes(dm.id) ? "bg-emerald-600 border-emerald-600" : "border-white/20 hover:border-emerald-500"
                                                    )}
                                                >
                                                    {selectedIds.includes(dm.id) && <Check className="w-4 h-4 text-white" />}
                                                </div>
